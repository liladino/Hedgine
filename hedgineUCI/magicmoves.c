...
u64 initmagicmoves_occ(const int* _squares, const int numSquares, const u64 linocc)
{
	int i;
	u64 ret=0;
	for(i=0;i<numSquares;i++)
		if(linocc&(((u64)(1))<<i)) ret|=(((u64)(1))<<_squares[i]);
	return ret;
}

u64 initmagicmoves_Rmoves(const int _square, const u64 occ)
{
	u64 ret=0;
	u64 bit;
	u64 rowbits=(((u64)0xFF)<<(8*(_square/8)));
...
u64 initmagicmoves_Bmoves(const int _square, const u64 occ)
{
	u64 ret=0;
	u64 bit;
	u64 bit2;
...
void initmagicmoves(void) {
	...
	for(i=0;i<64;i++)
	{
		...
		{
			u64 bit=temp&-temp;
			_squares[num_squares++]=initmagicmoves_bitpos64_database[(bit*0x07EDD5E59A4E28C2ULL)>>58];
			temp^=bit;
		}
		for(temp=0;temp<(((u64)(1))<<num_squares);temp++)
		{
			u64 tempocc=initmagicmoves_occ(_squares,num_squares,temp);
			magicmovesbdb[i][((tempocc)*magicmoves_b_magics[i])>>55]=initmagicmoves_Bmoves(i,tempocc);
		}
	}
	...
	for(i=0;i<64;i++) {
		...
		{
			u64 tempocc=initmagicmoves_occ(_squares,num_squares,temp);
			magicmovesrdb[i][((tempocc)*magicmoves_r_magics[i])>>52]=initmagicmoves_Rmoves(i,tempocc);
		}
	}
}
